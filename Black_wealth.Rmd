...
There is no one universally-accepted definition of wealth within the Black community and, therefore, no direct data on Black Wealth in Louisville. This report looks at one source of national data on monetary wealth and then looks at local data in three areas that are closely related to wealth and wealth-building: business ownership, housing finance, and income. 
...
At the county level, business ownership by race is only available for "employer businesses," businesses that have at least one employee. The data shows a a large racial disparity in ownership of employer businesses in Louisville. While employer businesses only make up about 20% of local registered businesses, they comprise 98.2% of local business revenue.

Data are available for all businesses (including businesses without employees) for the Louisville MSA, which includes 9 surrounding counties. While Black residents only own 2.3% of employer businesses in Louisville, they own 9.8% of all businesses in the larger Louisville MSA.
...
The map below shows business filings throughout 2020 within Jefferson County. These businesses include any type of LLC, corporation, or nonprofit officially registered with the state of Kentucky. Compared to the previous data that focuses on larger organizations, it shows that entrepreneurship is widspread throughout the city.
...
all_companies <- read_tsv("AllCompanies20210430.txt",
...
ggplot(left_join(map_zip, all_companies_zip, by = "zip")) + 
  ...
  scale_fill_viridis(name = "Companies per\n100 Residents") +
...
While housing represents a significant component of wealth. Housing reflects both historical wealth patterns––parental wealth makes it easier to buy a home––and is a major generator of wealth as, historically, homes have risen in value, increasing the wealth of those who own them. 
...
```{r}
devaluation <- read_csv("devaluation.csv")
...
The Jefferson County Property Value Administration (PVA) assesses homes every three years to determine their value, and that value is used to determine the amount of property taxes the owner owes. The PVA compares homes to similar nearby properties to determine an appropriate valuation. Based on a comparison of property tax assessments to actual sale prices, data shows that property valuations tend to overvalue homes in some neighborhoods and undervalue them in others, resulting in different property tax rates for owners.

### Property Assessments

The map below compares property assessments to market values. A 100 means that the average property assessment in that neighborhood matched the home's market value, while a value of 90 means that the assessment was only 90% of the home's market value.

Properties inside the Watterson and East of I-65 are consistenly valued, and thus taxed, at a lower rate than they are worth. Most over-assessments can be found in Southwest and South Louisville.

```{r}
# Load Property tax analysis webpage
page <- read_html("An Evaluation of Property Tax Regressivity in Jefferson County, Kentucky.html") 
...
make_map_bins <- function(indicator, 
                     ...
                     no_legend = FALSE, vir_dir = 1, bins){
  plt <- ggplot(jfco_sf) +
...
make_map_bins(sales_ratio_bins, 
              title = "Assessed Value compared to Market Value",
              legend = "Percent",
              bins = c(85, 90, 95, 100, 105, 110),
              caption = "Source: Greater Louisville Project
...
This image summarizes the previous two sections. If homes in Black neighborhoods were valued by the market and taxed by the city like homes in other neighborhoods, homeowners in those neighborhoods would have higher-priced homes taxed at a lower rate.
