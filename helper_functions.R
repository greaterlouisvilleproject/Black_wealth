...
if (FALSE) {
  ...
  #      poverty_nh, population_nh, file = "raw_data/glptools_exports.RData")
}
...
plt_by <- function(df, group_var, var, title_text = "Home Mortgages", subtitle_text, school = F,
                   y_axis = "Percent", y_min = NA, ymax = NA, units = "number",
                   caption_text = "", title_scale = 1,
                   remove_legend_title = F, legend_title = F) {
  ...
  plt <- ggplot(data = df, aes(x = year, y = !!var, group = !!group_var, colour = !!group_var)) +
    ...
    geom_line(size = .65) +
    theme_bw() +
    labs(title = title_text, y = y_axis) +
    theme(legend.position = "bottom")

  color_pal <- c("#323844", "#d63631", "#eaab21", "#a7bfd7", "#7CE3B6")[1:length(unique(pull(df, !!group_var)))]

  color_names <- c("Total", "Black", "White", "Hispanic", "Asian")
  
  color_names <- color_names[color_names %in% pull(df, !!group_var)]
  ...
  names(color_pal) <- color_names
  
  if (school) {
    label_vals <- seq(min(df$year), max(df$year), by = 1)
    label_labs <- paste0(label_vals - 1, "-", label_vals-2000)

    names(label_vals) = label_vals

    plt <- plt + scale_x_continuous(breaks = label_vals, labels = label_labs) +
      labs(x = "School Year")
  } else {
    plt <- plt + labs(x = "Year")
  }
  ...
  plt <- plt  +
    scale_colour_manual(breaks = names(color_pal),
                        values = color_pal) +
  ...
  if (legend_title) {
    plt <- plt + theme(legend.title = element_text(size = 30 * text_scale))
  } else {
    plt <- plt + theme(legend.title = element_blank())
  }

  if (!is.na(y_min)) {
    plt <- plt + ylim(y_min, ymax)
  }

  if (remove_legend_title) plt <- plt + theme(legend.title = element_blank())

  if(caption_text != "") {
    plt <- plt + labs(caption = caption_text) +
...
trend_cc <- function(df, var,
  ...
  if (y_title %in% c("Dollars", "Hourly Wage")) {
  ...
  p <- p + theme(
    ...
    plot.caption = element_text(size = 10 * txt_scale,
                                lineheight = 0.5))
